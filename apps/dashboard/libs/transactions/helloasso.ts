...
export async function fetchHelloAssoTransactions(
  { from } = { from: "2020-01-01T00:00:00Z" },
): Promise<any[]> {
  const PAGE_SIZE = "100";
  ...
  const formattedTransactions: FormattedTransaction[] = payments.map(formatHelloAssoTransactions);
...
async function fetchHelloAssoToken({
  ...
  await fetch(endpoint_access_token, {
    ...
    .then(({ access_token }) => {
      console.log("✅ Récupération d'un token HelloAsso réussie");
...
async function fetchHelloAssoTransactionsAndToken(
  ...
  console.log("🔁 Récupération d'une page de paiements \r\n");
  await fetch(endpoint_payments, {
    ...
    .then(({ data, pagination: { continuationToken } }) => {
      console.log("✅ Récupération d'une page de paiements réussie \r\n");
      payments = data;
...
function formatHelloAssoTransactions({
  ...
  items,
  state,
}: HelloAssoTransaction): FormattedTransaction {
