...
export default ({
  ...
  const progressRoundedFirstDecimal = Math.round(
    ((total + (totalTipeee === 0 ? 9192 : totalTipeee)) * 100) / goal,
  );

  const differenceInTime = new Date().getTime() - begin.getTime();
  const differenceInDays = Math.ceil(differenceInTime / (1000 * 3600 * 24));
  const meanDay =
    (total + (totalTipeee === 0 ? 9192 : totalTipeee)) / differenceInDays;
  return (
    <>
      <Card className="min-w-[350px] overflow-scroll border-none bg-black/90 text-white shadow-lg backdrop-blur-md">
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <p className="text-xl">
              {inEuros(totalTipeee === 0 ? 9192 : totalTipeee)}
            </p>
          </div>
          {/* Total + Tipeee */}
          <div>
            ...
            <p className="text-xl">
              {inEuros(total + (totalTipeee === 0 ? 9192 : totalTipeee))}
            </p>
          </div>
          {/* Progress */}
          <div>
